...
3. **Check the Dashboard:** Navigate to the **"Agent API Monitoring"** Grafana Dashboard.
   - **Review "Request Rate":** Is the total traffic volume normal?
   - **Review "Rejections by Reason":** Identify which reason is triggering the spike.
...
**1. Search for rejected requests**
```logql
# Example
...
| :--- | :--- | :--- |
| **Spike tracks exactly with a new Prompt Deployment (`prompt_version` changed)** | recent update to the agent's prompt or ruleset making it overly sensitive. | Roll back to the previously stable `prompt_version`. Notify the AI team during business hours. |
| **Spike in Rejections + Massive Overall Traffic Spike** | Adversarial attack, scraping, or bot net sending malicious prompts. | Do not roll back. Implement rate-limiting or WAF rules |
| **Rejections are heavily skewed to a single `reason`** | Upstream model change, or user behavior is legitimately shifting (e.g., a viral trend triggering a false positive safety rule). | Inform the AI Team (On-call). The safety guardrail may need hotfixing. Inform customer support of potential false-positive blocks. |
